...
public class PostAccesoDatos {
	...
	public static List<Post> obtenerTodos() {
		return enTransaccion(em -> em.createQuery("select p from Post p order by p.fecha desc", Post.class).getResultList());
	}
	...
	public static List<Post> obtenerPorUsuario(long idUsuario) {
		return enTransaccion(em -> em.createQuery("select p from Post p order by p.fecha desc where p.usuario.id = :idUsuario", Post.class).setParameter("idUsuario", idUsuario).getResultList());
	}
}
